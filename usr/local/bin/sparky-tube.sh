...
echo -e "
                ...
                $(find $LANGUAGE_FILES/ -iname sparky-tube.mo | cut -d/ -f5 | sort | paste -d, -s - - | fold -s -w 80)
...
#     find $LANGUAGE_FILES/ -iname sparky-tube.mo: Encontra todos os arquivos com o nome sparky-tube.mo dentro do diretório especificado por $LANGUAGE_FILES/.
#     cut -d/ -f5: Extrai a quinta coluna do caminho dos arquivos, supondo que as pastas do caminho estejam separadas por /.
#     sort: Ordena as entradas alfabeticamente.
#     paste -d, -s - -: Junta todas as entradas em uma única linha, separando-as por vírgulas.
#     fold -s -w 80: Quebra a linha em múltiplas linhas, com largura máxima de 80 caracteres.
...
_getFileName() {
    ...
    if [ -z "$FILE" ]; then
        ...
        echo "$(gettext 'Error: File name not found!')"
    ...
    echo "$(gettext 'Final file name'): $PLAYLISTITLE"
}
...
if echo "$URL" | grep -Eqs '&list' ; then
    FILE="$(gettext 'Downloading from playlist to') $PLAYLISTITLE"
...
# -i "$logo": Adiciona a imagem ou ícone à notificação. A variável $logo deve conter o caminho para o arquivo de imagem.
    ...
    if [ $? -ne 0 ]; then
        ...
        yad --center --window-icon="$logo" --image=dialog-error --timeout="10" --no-buttons \
            --title="$(gettext 'Notice')" --text="$(gettext 'Failed to download audio. Please try again later.')" --width="450" --height="100"
    fi
fi
...
echo -e "\n$(gettext "In \"Media Player\" was used: \"$PLAYLIST\"") \n"
     ...
     # echo   "$PLAYLST $PLAYLISTITLE.$PLAYFORMAT"


     "$PLAYLST" "${FILE%.*}.$PLAYFORMAT" 2>/dev/null &

     # "$PLAYLST" "$PLAYLISTITLE.$PLAYFORMAT" 2>/dev/null &
